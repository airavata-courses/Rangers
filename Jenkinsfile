pipeline{
	...
	environment {
	  ...
	  stages {
	        ...
	        stage('Clone Repo'){
	            steps{
	                git branch: 'notification_blue', url: 'https://github.com/airavata-courses/Rangers.git'
	            }
	        }
	        stage('Deploy'){
	            steps{
			...
			docker image prune -a -f
					...
					docker login --username=DOCKER_USERNAME --password=DOCKER_PASSWORD || true
            	    			...
            	    			docker tag $id chaitrali1805/notification-service-blue:latest
                    			docker push chaitrali1805/notification-service-blue:latest
					...
					JENKINS_NODE_COOKIE=dontKillMe nohup ssh -tt ubuntu@149.165.171.144 '
			    		sudo su<<EOF
			    		kubectl delete deployment notificationdeployment-blue
                            		sleep 30
                            		kubectl apply -f notification-deploy-blue.yaml
					sleep 20
